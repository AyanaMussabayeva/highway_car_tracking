...
class CarExtractor():
    def __init__(self, background_sub: BackgroundSubstraction, 
                 erode_kernel_size:int, dilate_kernel_size:int,
                min_contour_area):
        self.background_sub = background_sub
        self.erode_kernel = np.ones(
    ...
    def _get_mask(self, mask):
        ...
        contours, hierarchy = cv2.findContours(image=morph_mask,
                                               mode=cv2.RETR_EXTERNAL,  
                                               method=cv2.CHAIN_APPROX_NONE)
        image_copy = morph_mask.copy()   
        selected_contours = [c for c in contours if cv2.contourArea(c) > self.min_contour_area]
    ...
    def report(self):
        ...
        for idx, cur_frame_bboxes in enumerate(self.selected_bboxes):
            for bbox_idx, bbox in enumerate(cur_frame_bboxes):
                bbox_data = bbox_data.append({'bbox_xywh': bbox, 'frame' : int(idx)}, ignore_index = True)
